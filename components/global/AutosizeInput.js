...
class AutosizeInput extends Component {
  ...
  updateInputWidth() {
    ...
    if (
      ...
      newInputWidth =
        Math.max(this.sizer.scrollWidth, this.placeHolderSizer.scrollWidth) + 5
    } else {
      newInputWidth = this.sizer.scrollWidth + 8
    }
  ...
  onBlur(e) {
    ...
    setTimeout(function() {
      if (!currentTarget.contains(document.activeElement)) {
        const enterKeyEvent = document.createEvent('KeyboardEvents')
        enterKeyEvent.initKeyboardEvent(
          'keydown',
          true,
          true,
          null,
        ...
        currentTarget.dispatchEvent(enterKeyEvent)
      }
    }, 0)
  }
  renderStyles() {
  ...
  render() {
    ...
    return (
      <div
        className={'autosize-input ' + (this.props.className || '')}
        style={wrapperStyle}>
        {this.renderStyles()}
        <input
          {...inputProps}
